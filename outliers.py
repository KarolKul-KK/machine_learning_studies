...
def read_csv() -> pd.DataFrame:
    ...
    df = pd.read_csv('houses_data.csv', usecols=['Price', 'Bathroom', 'Landsize', 'Propertycount', 'Postcode'], dtype={'Price': float, 'Bathroom': int, 'Landsize': float, 'Propertycount': float, 'Postcode': float})
...
def IQR_outliers(df: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
    ...
    for i in range(len(df)):
        if ((df.iloc[i] < (Q1 - 1.5 * IQR))|(df.iloc[i] > (Q3 + 1.5 * IQR))).all() == True:
            pass
        else:
            df_new.drop(i, inplace=True)
